            ...
            <div class="col-xs-7 col-sm-9 col-md-9 col-lg-9">                
                <table class="table">
                    ...
                    <tr>
                        ...
                        <td class="text-left" style="width:60%">
                            <label class="ns" style="color:gray">{{date_format(date_create(Auth::user()->birthday),"d/m/Y")}}</label>
                            <input type="date" id="ns" class="display-none form-control" value="{{date_format(date_create(Auth::user()->birthday),"d/m/Y")}}" ns-before="{{date_format(date_create(Auth::user()->birthday),"d/m/Y")}}" />
                        </td>
                        <td class="text-right" style="width:20%">
                            <a href="#" class="ns-e btn" onclick="$('.ns-s, .ns-c, #ns').removeClass('display-none');$('.ns-e, .ns').addClass('display-none')">Chỉnh sửa</a>
                            <a href="#" class="ns-s btn display-none" onclick="changeNgaySinh()">Lưu </a>
                            <a href="#" class="ns-c btn display-none" onclick="$('.ns-s, .ns-c, #ns').addClass('display-none');$('.ns-e, .ns').removeClass('display-none');$('.ns').html($('#ns').attr('ns-before'));$('#ns').val($('#ns').attr('ns-before'));"> Hủy</a>
                        </td>
                    </tr>
                </table>
            </div>
...
<script>
    $('#user-password').iziModal({
        ...
        onOpening: function(){
            ...
            $('#re-pw2').val('');
            $('.rs-pw').html('');
            $('.rs-pw').removeClass('fa-check');
            $('.rs-pw').removeClass('fa-close');
        }
