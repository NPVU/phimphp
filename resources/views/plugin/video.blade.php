...
<div class="col-xs-2 col-sm-2 col-md-2 col-lg-2">
    ...
    <script type="text/javascript">$(document).ready(function(){$('#video-player').load();$.ajax({url: '{{url("/autoload")}}/?pid={{$_GET['pid']}}&t={{$_GET['t']}}&token={{Session::token()}}',dataType: 'text',type: 'get',success:function(data){if(data!=null){data = JSON.parse(data);$('#video-player').attr('src', data['360p']);setTimeout(function(){document.getElementById('video-player').play();},1000);$('#google360p').attr('src', data['360p']);if(data['720p']!=null){$('.icon-quality').removeClass('display-none');$('#google720p').attr('src', data['720p']);}if(data['1080p']!=null){$('#google1080p').attr('src', data['1080p']);}$('#video-player').removeAttr('poster');$('#video-player').prop('controls',true);}}});});var video = document.getElementById('video-player');var v = 0;$('.npv-play').click(function(){if(video.paused){video.play();}else{video.pause();}});$('.npv-quality').click(function(){var currentTime = video.currentTime;if($('.npv-quality').attr('quality') === "360"){console.log('720');video.setAttribute('src', $('#google720p').attr('src'));$('.npv-quality').css('color','white');$('.npv-quality').attr('quality', "720");video.onerror = function(){video.setAttribute('src', $('#google360p').attr('src'));video.currentTime = currentTime;video.play();};}else{console.log('360');video.setAttribute('src', $('#google360p').attr('src'));$('.npv-quality').css('color','gray');$('.npv-quality').attr('quality', "360");}video.currentTime = currentTime;video.play();});video.onplaying = function(){$('.npv-play > i').addClass('fa-pause');$('.npv-play > i').removeClass('fa-play');if(v===0){v=1;setTimeout(function(){viewTimes("{{url('update')}}/{{strtolower(str_replace(' ', '-',ClassCommon::removeVietnamese($phim[0]->phim_ten)))}}/?pid={{$_GET['pid']}}&t={{$_GET['t']}}&s={{md5('google')}}&token={{csrf_token()}}");}, 10000);}};video.onpause = function(){$('.npv-play > i').addClass('fa-play');$('.npv-play > i').removeClass('fa-pause');}</script>    
</div>
@elseif(strcmp($_GET['s'], md5('youtube'))==0)
    ...
    @else    
    <script>
        window.location.href = "{{url('xem-phim')}}/{{strtolower(str_replace(' ', '-',ClassCommon::removeVietnamese($phim[0]->phim_ten)))}}/?pid={{$_GET['pid']}}&t={{$_GET['t']}}&s={{md5('google')}}&token={{csrf_token()}}";
    </script>
    @endif
@else
