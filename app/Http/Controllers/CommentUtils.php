...
class CommentUtils extends BaseController
{
    ...
    public static function getHTMLComment($phim_id, $limit, $offset){
        ...
        if(count($listComment)>0){
            foreach ($listComment as $row){
                $subComment = DB::select(DB::raw('SELECT users.name, users.email, users.avatar, binhluan.*, (SELECT count(1) FROM users_roles WHERE users_roles.user_id = binhluan.user_id) AS role FROM users, binhluan WHERE binhluan.user_id = users.id AND binhluan.phim_id ='.$phim_id.' AND binhluan_id_cha = '.$row->binhluan_id.' ORDER BY binhluan_id DESC'));
                $subHTML = '';
                $html   .= '<tr><td style="width:60px; height:60px;vertical-align:top" '.($isAdmin?'oncontextmenu="openContextMenu('.$row->binhluan_id.')"':'').'>';
                if($row->active == 1){
                    $html   .=      '<img src="'.URL('/').'/'.$row->avatar.'" class="avatar img-circle" width="60" height="60" />';
                    $html   .=      '</td><td class="text-left">'; 
                    $html   .=      '<div><span class="username-comment '.($row->role>0?'color-name-admin':'').'" style="font-weight: 700;">'.($isAdmin?$row->name.' ('.$row->email.')':$row->name).'</span><span style="float:right;">'.ClassCommon::getStrSoNgayDaQua($row->binhluan_ngaycapnhat).'</span></div>';
                    $html   .=      '<div><span class="content-comment '.($row->role>0?'color-comment-admin':'').'">'.$row->binhluan_noidung.'</span>';
                    if($hasLogin){
                    ...
                    if(count($subComment)>0){
                        ...
                        foreach($subComment as $sub){
                            $subHTML   .= '<tr><td style="width:60px; height:60px;" '.($isAdmin?'oncontextmenu="openContextMenu('.$sub->binhluan_id.')"':'').'>';
                            $subHTML   .=      '<img src="'.URL('/').'/'.$sub->avatar.'" class="avatar img-circle" width="100%"/>';
                            $subHTML   .=      '</td><td class="text-left">';                                        
                            $subHTML   .=      '<div><span class="username-comment '.($sub->role>0?'color-name-admin':'').'" style="font-weight: 700;">'.($isAdmin?$sub->name.' ('.$sub->email.')':$sub->name).'</span><span style="float:right;">'.ClassCommon::getStrSoNgayDaQua($sub->binhluan_ngaycapnhat).'</span></div>';
                            $subHTML   .=      '<div class="content-comment '.($sub->role>0?'color-comment-admin':'').'">'.$sub->binhluan_noidung.'</div>';
                            $subHTML   .= '</td></tr>';   
                        }
                        $subHTML .= '</table>';
                    }
                    ...
                    $html   .=      '</td><td class="text-left">'; 
                    $html   .=      '<div><span class="username-comment color-name-locked" style="font-weight: 700;">Tài khoản bị khóa</span><span style="float:right;">'.self::getStrSoNgayDaQua($row->locked_at).'</span></div>';
